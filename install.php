<?php
/**
 * Carlos POS Web — one-click installer
 * ------------------------------------------------------------
...
$sqlFile  = __DIR__ . '/database/Carlos_pos.sql';
...
if (is_post()) {
    if (!csrf_ok()) {
        ...
        $name = trim((string) post('db_name', 'Carlos_pos'));
        ...
        if (!$errors) {
            try {
                ...
                if (!file_exists($sqlFile)) {
                    throw new RuntimeException('database/Carlos_pos.sql was not found.');
                }
  ...
  <div class="auth-card" style="width:min(560px,100%)">
    ...
    <?php else: ?>
      ...
      <p class="muted small" style="margin-top:1rem">
        Prefer manual setup? Import <code>database/Carlos_pos.sql</code> in phpMyAdmin and edit
